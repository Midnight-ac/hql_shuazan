...
def main2():
    ...
    localtime = time.asctime(a)
    print("当前时间：" + time.asctime(a))
...
def get_comment():
    ...
    if any((blackWord in comment for blackWord in blackWords)):
        print(f'{red}检测到违规词{blackWords}{end}')
        sys.exit()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                print(e)
def cttj():
    c = input(f"{yellow}请输入你想添加的词条：{end}")
    bad_words.append(c)
    print(f"{green}添加成功!{end}现在的词条是:",bad_words)
...
def _getHeader(target_url: str, useAndroid: bool=False):
    ...
    return headers
funcs = [get_comment, get_like, bad_comment]
print(' 导入主程序中...', end='\r')
import threading
import os
import re
...
def main():
    while True:
            ...
            print("欢迎使用黄清凉刷取工具!")
            main2()
            print(f"\033[31mctrl + c强制退出{end}\n\033[33m[1] 刷赞\n{blue}[2] 刷评论{end}\n\033[33m[3] 添加评论词条\n{blue}[4] 清除所有词条{end}\n\033[35m[5] 刷流量")
            break
    li = ["1","2","3","4","5"]
    try:
        while True:
            time.sleep(0.5)
            a = input("\033[31m请选择你想要使用的操作：\033[0m")
            if a in li and a == "5" and a.isdigit():
                ...
                print(f"\033[31mctrl + c强制退出{end}\n\033[33m[1] 刷赞\n{blue}[2] 刷评论{end}\n\033[33m[3] 添加评论词条\n{blue}[4] 清除所有词条{end}\n\033[35m[5] 刷流量")
                    ...
                    bad_comment()
                    print(f"\033[31mctrl + c强制退出{end}\n\033[33m[1] 刷赞\n{blue}[2] 刷评论{end}\n\033[33m[3] 添加评论词条\n{blue}[4] 清除所有词条{end}\n\033[35m[5] 刷流量")
            elif a in li and a == "3" and a.isdigit():
                    cttj()
                    print(f"\033[31mctrl + c强制退出{end}\n\033[33m[1] 刷赞\n{blue}[2] 刷评论{end}\n\033[33m[3] 添加评论词条\n{blue}[4] 清除所有词条{end}\n\033[35m[5] 刷流量")
            elif a in li and a == "4" and a.isdigit():
                    qclb()
                    print(f"\033[31mctrl + c强制退出{end}\n\033[33m[1] 刷赞\n{blue}[2] 刷评论{end}\n\033[33m[3] 添加评论词条\n{blue}[4] 清除所有词条{end}\n\033[35m[5] 刷流量")
            elif a not in li:
                    print("目标不存在")
